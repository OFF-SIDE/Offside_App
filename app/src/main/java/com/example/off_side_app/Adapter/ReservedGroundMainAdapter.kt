...
class ReservedGroundMainAdapter: RecyclerView.Adapter<ReservedGroundMainAdapter.ReservedGroundViewHolder>() {
    private var items = listOf<ReservedGroundInfoGroup>()

    inner class ReservedGroundViewHolder(private val binding: ReservedGroundRecyclerviewHeaderBinding):
        RecyclerView.ViewHolder(binding.root){
        fun bind(group: ReservedGroundInfoGroup){
            binding.locationHeaderText.setText(group.location)
            binding.rvGroupedReservedGround.apply {
                setHasFixedSize(true)
                layoutManager = LinearLayoutManager(binding.root.context)
                adapter = ReservedGroundMainItemAdapter(group.groupedReservedGround)
                addItemDecoration(
    ...
    //
    fun setList(ground: List<ReservedGroundInfoGroup>) {
        items = ground
    }
}